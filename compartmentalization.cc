...
template <int dim>
auto
Compartmentalization<dim>::read_temperature()
-> std::vector<double>
{
    // Read the velocity magnitude and write it in a vector
    std::vector<double> temperature;
    std::ifstream       fin("temperature");
    double              element;
    while (fin >> element)
    {

        temperature.push_back(element);
    }
  ...
  for (const auto &pair : overlaid_set)
    {
      ...
      for(typename Triangulation<dim>::active_cell_iterator d : pair.second)
        {
           sum= sum + (map_cell_electric_field[d]*map_cell_electric_field[d]);
        }

      double average_square = sum / pair.second.size();
      double average= sqrt(average_square);
  ...
  for (const auto &pair : overlaid_set)
    {
      ...
      inlet_flux_to_id1.push_back(in_flow);
...
Compartmentalization<dim>::output_flux()
{
  ...
  inlet_flux_to_id1.push_back(0);
  // push it to matrix_of_flux
  matrix_of_flux.push_back(inlet_flux_to_id1);
...
template <int dim>
void
Compartmentalization<dim>::write_file_temperature(
        ...
        const unsigned int &step_number)
{
    ...
    DataOut<dim> data_out;
    data_out.attach_triangulation(triangulation);
    std::string average_solution_names("Temperature");

    data_out.add_data_vector(Temperature,
                             average_solution_names,
                             DataOut<dim>::type_cell_data);
...
template <int dim>
void
Compartmentalization<dim>::temperature_contour()
{
    std::vector<double> temperature;
    std::ifstream       fin("temperature");
    double              element;
    ...
    Vector<double> compartments_temperature;
    compartments_temperature.reinit(triangulation.n_active_cells());
    int i=0;
    for (const auto &pair : overlaid_set)
    {
        for (typename Triangulation<dim>::active_cell_iterator d : pair.second)
        {
            compartments_temperature[d->active_cell_index()] = temperature[i];
        }
        std::cout<<temperature[i]<<std::endl;
        i++;
    }
    write_file_temperature(compartments_temperature, 1.0, 1);
}
